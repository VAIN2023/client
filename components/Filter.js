    ...
    return (
        <div className="w-3/12">
			...
			<div className="w-full bg-white border-slate-300 border rounded-md drop-shadow-md p-5 mt-5">
				...
				{
					subjects.map((subject, index) => (<p key={index}><input type="checkbox" className="text-neutral-800 mr-2" onChange={handleSubjectChange} checked={subjectsChecked.includes(subject.subjectdescription)} value={subject.subjectdescription}/>{subject.subjectdescription}</p>))
				}
				<h1 className="font-bold text-xl mt-5 text-neutral-800 mb-2">Types</h1>
				{
					types.map((type, i) => (<p key={i}><input type="checkbox" className="text-neutral-800 mr-2" onChange={handleTypeChange} checked={typesChecked.includes(type.typedescription)} value={type.typedescription} />{type.typedescription}</p>))
				}
